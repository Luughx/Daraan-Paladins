...
const historyFunction = async (client, message, api, player, type) => {
    const history = await api.getPlayerMatchHistory(player)
    if (!history[0].Match_Time) return await message.channel.send("Matchs not found or the account is private")
    ...
    if (type) {
        for (let i=0; i < history.length; i++) {
            if (history[i].Queue.toLowerCase() == type.toLowerCase()) {
    ...
    if (pagesType.length == 0) return await message.channel.send("El jugador no tiene partidas recientes en ese modo de juego")
 ...
 */
    ...
    const pages = pageInfo(message, pagesType, totalPages, championsRoles)
    ...
    const embed = historyEmbed(client, message, pages, index, totalPages, history, data)
    let currentPage
    totalPages == 1 ? currentPage = await message.channel.send({ embeds: [embed] }) : currentPage = await message.channel.send({ embeds: [embed], components: [buttons], fetchReply: true}) 
    ...
    collector.on("collect", async i => {
        ...
        if (i.user.id !== message.author.id) return await i.deferUpdate()
        let embedActu
        ...
        embedActu = historyEmbed(client, message, pages, index, totalPages, pagesType, data)
        
        await currentPage.edit({ embeds: [embedActu], components: [buttons]})
        collector.resetTimer()
        await i.deferUpdate()
    })

    collector.on("end", async i => {
        await currentPage.edit( {embeds: [ historyEmbed(client, message, pages, index, totalPages, history, data)], components: []} )
    })
}

const pageInfo = (message, pagesType, totalPages, championsRoles) => {
...
const historyEmbed = (client, message, pages, index, totalPages, history, data) => {
