...
const detailedFunction = async (client, int, api, player) => {
    const matchIdOption = int.options.get("match_id")
    let matchId

    if (matchIdOption) {
        matchId = matchIdOption.value
    } else {
        const history = await api.getPlayerMatchHistory(player)
        if (!history[0].Match_Time) return await int.editReply("El usuario no tiene partidas recientes")
        matchId = history[0].Match
    }

    const matchData = await api.getMatchDetails(matchId)
    if (matchData.length == 0) return await int.editReply("Match not found")
    ...
    if (!match) return await int.editReply("This user did not play in that match")
